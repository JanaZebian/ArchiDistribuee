...
class BookingServicer(booking_pb2_grpc.BookingServicer):
    ...
    def GetBookingByUserID(self, request, context):
        ...
        return booking_pb2.BookingData(userid="!not found!", dates=[])
    ...
    def AddBookingByUser(self, request, context):
        ...
        with grpc.insecure_channel('localhost:3002') as channel:
            ...
            time_req = stub.GetMovies(showtime_pb2.Date(date=request.date))
            if time_req is None:
                            ...
                            if str(bk_date["date"]) == str(request.date):
                                ...
                                for available_movie_id in bk_date["movies"]:
                                    if str(available_movie_id) == str(request.movieid):
                                        movie_found = True
                                        return booking_pb2.Id(
                                            id="error : the requested movie is already booked by that user")
